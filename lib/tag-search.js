...
class Tagged extends React.Component {
	...
	useEvents(events) {
		...
		events.removeListener('tag-search:update', this._eventUpdate);	
		events.removeListener('tag-search:config', this._eventConfig);
		events.removeListener('tag-search:tag', this._eventTag);
		
		// add new listeners
		events.on('tag-search:update', this._eventUpdate);	
		events.on('tag-search:config', this._eventConfig);
		events.on('tag-search:tag', this._eventTag);			
	}
	_eventUpdate(cfg) {
	...
	_eventConfig() {
		this.props.events.emit('tag-search:options', this.state.Anchored)
	}
	_eventTag(add) {
		this.addAnchors(add);
	}
	...
	addAnchors(add) {
		...
		debug('add anchors', tags, where, $hs)
