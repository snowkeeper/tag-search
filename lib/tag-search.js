...
class Tagged extends React.Component {
	constructor(props){
		...
		// classes
		this.state.Anchored.classes = this.state.Anchored.noclasses ? {} : _.defaults(opts.classes || {}, defaultClasses.classes)
		// styles
		this.state.Anchored.styles = this.state.Anchored.nostyles ? {} : _.defaults(opts.styles || {}, defaultStyles.styles)
		...
		$(document).on('input focus', '#' + thisComponent.state.Anchored.searchBar + ' input', function(e) {
			...
			_.forEach($allAnchors, function(v) {
				...
				if (_.isString(thisComponent.state.Anchored.nameFromTag) && thisComponent.state.Anchored.nameFromTag.indexOf($next.prop("tagName")) > -1) {
					nameFromTag = $next.clone().children(thisComponent.state.Anchored.nameFromTagSaveChildren).remove().end().text()
				}
	...
	catchMenuClick (e) {
		...
		this.$searchInput.removeClass('active caution')
	}
	rateLimited(id, time) {
		/**
