...
class Tagged extends React.Component {
	constructor(props){
		...
		this.state.AnchorSearch =  this.setTagComponent()
	...
	updateConfig(cfg) {
		this.setState({
			Anchored: _.merge(this.state.Anchored, cfg)
		})
	}
	render() {
	...
	searchTags(search) {
		...
		$allAnchors.each(function(k, v) {
			...
			// set display names
			let name = $anchor[0][config.nameFromTagAttr] || '';
			...
			} else if(thisComponent.state.nameFromPrevTag) {
				const $next = $anchor.prev()
			} else {
				const $next = $anchor
			}
			if (('string' === typeof config.nameFromTag) && config.nameFromTag.indexOf($next.prop("tagName")) > -1) {
